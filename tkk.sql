...
-- 1) 게시판 글의 DELETED_YN 현황 확인
SELECT DELETED_YN, COUNT(*) AS CNT
FROM TKK.BOARD_POST
GROUP BY DELETED_YN;

-- 2) 거래글의 DELETED_YN 현황 확인
SELECT DELETED_YN, COUNT(*) AS CNT
FROM TKK.TRADE_POST
GROUP BY DELETED_YN;

UPDATE TKK.TRADE_POST
SET DELETED_YN = 'N'
...
ALTER TABLE TKK.POPUP_STORE
    ADD DELETED_YN CHAR(1) DEFAULT 'N' NOT NULL;

UPDATE TKK.POPUP_STORE
SET DELETED_YN = 'N'
WHERE DELETED_YN IS NULL;

COMMIT;

SELECT popup_id, deleted_yn
FROM TKK.POPUP_STORE
WHERE ROWNUM <= 5;
