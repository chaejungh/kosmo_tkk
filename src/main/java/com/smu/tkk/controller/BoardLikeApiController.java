...
public class BoardLikeApiController {
    ...
    public LikeResponse toggle(@PathVariable Long postId,
        ...
        isNewLike = result.getId() != null;

        if (isNewLike) {
            BoardPost post = boardService.readOne(postId);
            Long writerId = post.getMember().getId();

            Optional<MemberNotificationSetting> settingOpt = memberNotificationSettingService.get(writerId);
            System.out.println(settingOpt.get());
            if (settingOpt.isPresent() && "Y".equals(settingOpt.get().getLikeYn())) {
                ...
                notificationPublisher.send(writerId,"회원님 게시글에 좋아요가 눌렸습니다.");
            }
        }


        return new LikeResponse(result.getLikeCount(), isNewLike);
